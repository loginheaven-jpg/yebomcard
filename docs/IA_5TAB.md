...
> 2026-09-17 '목차' + '본문' 을 **성경** 하나로 합치고 **말씀의삶** 을 탭으로 돌려놓았다.
> (파일명은 이력 보존을 위해 IA_5TAB.md 를 유지한다)

---

## 0. 세 자리의 역할 (2026-09-17 지휘부)

| 자리 | 역할 | 항목 |
|---|---|---|
| **하단 탭** | 자주 가는 곳 — 이름표가 보여야 고령 교인이 찾는다 | 성경 · 검색 · 말씀의삶 · 찬송가 · 책갈피 · 설정 |
| **⋮ 메뉴** | 지금 이 말씀을 어떻게 볼까 + 자주 쓰는 계정 동작 | 글자 크기 · 한 절씩 크게 · 예배성경 · 앱으로 설치 · 로그인 |
| **설정** | 한 번 정해 두는 것 | 계정 · 화면 · 음성 · 앱 정보 |

- **전부 ⋮ 로 넣고 탭을 없애지 않는다**: 숨긴 메뉴는 덜 쓰이고, 예배 중 찬송가·본문은 한 번에 눌려야 하며,
  본문 몰입은 이미 탭바 자동 숨김이 맡는다
- **⋮ 을 누르면 하단 탭바가 함께 올라온다** — 메뉴가 두 무리라는 것을 보이게. 메뉴가 열린 동안 자동 숨김을 멈추고
  탭바를 메뉴의 바깥 닫기 막(z-59) 위로 올린다(`raised`, z-61). 그 사이 탭을 누르면 탭이 먹고 메뉴는 닫힌다(`moreMenuCloseNonce`)
- 로그인은 자주 쓰므로 ⋮ 에 둔다(지휘부). 설정의 '도구'(예배성경 · 성경카드 · 앱으로 설치)와 찬송가 큰 카드는 지웠다 —
  같은 것이 탭이나 ⋮ 에 있다. 앱으로 설치는 도구가 없어져 ⋮ 에만 남는다. 성경카드는 절 선택 → '이 말씀으로 카드 만들기'
...
| 탭 | 동작 |
|---|---|
| **성경** (bible) | 본문을 읽는 중이면 `navRequest.target="toc"`(목차), 그 밖에서는 `"read"`(마지막 읽던 곳, 없으면 목차). 판정은 page.tsx(`isReadingView`). 목차로 나가면 플랜 모드를 푼다 |
| **검색** (search) | `navRequest.target="search"` — 단어·주제 검색 모드 |
| **말씀의삶** (plan) | `setView("plan")` — 2026-09-17 탭으로 돌아왔다 |
| **찬송가** (hymn) | `setShowHymn(true)` — 창을 띄운다(view 변경 없음) |
| **책갈피** (bookmark) | `navRequest.target="bookmark"` — 책갈피 메뉴 노출 (+ 스크랩 진입) |
| **설정** (settings) | `SettingsSheet` 띄움 (view 변경 안 함) |

탭 강조는 화면을 보고 정한다 — `view === "plan"` 이면 말씀의삶. 말씀의삶은 탭 말고도 들어오는 길(플랜 헤더 '진도표로' ·
완료 시트 · 초대링크 · 로그인하고 돌아옴)이 있기 때문이다.

### 말씀의삶(plan 뷰)에서 주의할 것 세 가지

**하나 — plan 은 `navRequest` 를 쓰지 않는다.**
`handleTabChange` 가 `plan` 을 **`setView("search")` 보다 먼저** 받아 `setView("plan")` 하고 끝낸다.
순서가 바뀌면 검색 뷰만 뜬다(2026-09-12 이전의 함정). 회차를 읽는 중에는 `PlanHeader` 의 '진도표로' 로도 돌아온다.
...
2. ~~찬송가 · 예배성경 · 카드 빌더 · 앱으로 설치~~ — 2026-09-17 지웠다(§0). 찬송가는 탭, 예배성경·설치는 ⋮
