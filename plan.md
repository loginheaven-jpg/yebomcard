...
- **AI 게이트웨이 개선 의뢰(2026-09-18)** — 성경 질문이 느린 까닭의 약 70% 가 게이트웨이 쪽이라(요청마다 +9초 안팎),
  게이트웨이 담당에게 수정을 의뢰했다(지시서는 공개 저장소 밖에 둠). **보고를 받으면 같은 질문으로 전후 시간을 다시 재서 올린다.**
  그래도 부족하면 Vercel AI Gateway(BYOK) 를 검토한다 — Vercel 유료 가입은 필요 없고, 소액 선불 크레딧이 든다
- **배포 뒤 재 봐야 하는 것** — 프로덕션에서 질문 한 번의 실제 응답 시간. 이 PC 에서 게이트웨이까지는
  왕복 9~15초였는데 그 가운데 8초쯤이 이 PC 의 네트워크였다(게이트웨이가 잰 시간은 1~3.7초).
  Vercel 에서는 훨씬 짧을 것이나 **재 보지 않았다**
