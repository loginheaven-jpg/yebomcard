...
## 음원 절 끝이 짧게 끊기는 절 — 376절 (2026-09-12, 조사 필요)

새 방식으로 만든 5,545절 중 **376절(6.8%)** 이 절 끝 측정값 120ms 미만이다(정상 절은 220~1400ms).
창세기 1:1·1:2·1:3·1:5 가 그 예 — 지휘부가 듣고 "끝 글자가 짧게 끊어진다"고 지적한 절들이다.

- **침묵이 짧은 게 아니라 소리가 없다.** 파형을 보면 마지막 50ms 까지 소리가 살아 있는데 파일이 거기서 끝난다.
  정상 절은 소리가 잦아들며 끝난다(창세기 1:4). 재생 쪽에서 무음을 붙여도 메울 수 없다
- **재시도가 듣지 않는다.** 지금 '절 끝 짧음' 재시도는 **같은 조건으로 통째로 다시 만든다**(조각으로 나누면 절
  중간에 쉼이 끼어 호흡이 달라지므로 일부러 그렇게 두었다). 그래서 세 번 모두 같은 결과가 나오고
  '가장 나은 시도' 를 쓴다. 조건을 바꿔야 한다 — 후보: 본문 끝에 여백·문장부호를 덧붙여 모델이 급히 닫지 않게,
  온도를 올려 다른 표현을 얻기, 마지막 구절만 조각으로 만들어 이어 붙이기
- **먼저 실험이 필요하다.** GPU 가 교체 작업을 도는 동안에는 모델을 두 번 올릴 수 없다(12GB). 큐가 비는 때
  창세기 1:1 같은 절로 조건을 몇 가지 비교해 재본 뒤, 통하는 조건으로 376절을 한꺼번에 다시 만든다
- 문장이 통째로 빠지는 문제(연속 8자)는 2026-09-12 검수 강화로 따로 잡았다 — architecture.md 같은 날 항목
