...
| 파일 | 하는 일 |
|---|---|
| `성경 음원 이어하기.bat` | 개발본에서 ① `run_plan.py --voice-key f4 --testament new`(신약 남은 절) → ② ①이 정상으로 끝나면 `--testament old --replace-legacy`(구약 구방식 교체, 진도표 순서). 끝난 책과 이미 만든 절은 건너뛴다 |
| `성경 음원 중지.bat` | 도는 워커를 멈춘다(`run_plan.py --stop`). 진행 상황은 남고 다음에 이어간다 |

2026-09-11 분담 변경(아래 '구방식 음원 교체')으로 `신약 음원 이어하기/중지.bat` 을 대신했다. ②는 ①의 종료 코드가
0 일 때만 — 중지 배치는 워커를 강제 종료(코드 1)하므로 멈추면 ②로 넘어가지 않는다. 배치 파일엔 한글을 쓰지 않는다
(cmd 가 UTF-8 한글 줄을 잘못 읽는다 — 영문·숫자만, CRLF).
...
```
cmd /c start "" "C:\Users\예봄교회\Desktop\성경 음원 이어하기.bat"
```
...
- 이 PC: 바탕화면 '성경 음원 이어하기' — 신약 남은 절(`--testament new`) → 구약 교체(`--testament old --replace-legacy`) 자동
- 새 PC: 스튜디오 '2. 생성' 탭 교체할 쪽 **신약** → **[구방식 교체 작업 걸기]** — 한 번 눌러 두면 구약 남은 절이 끝난 뒤 진행.
  예전 지시대로 **구약** 교체를 걸어 두었다면 먼저 교체할 쪽 **구약** → **[교체 작업 걷기]** 로 거둔다
  (그 쪽 교체 작업 중 아직 만들 절이 남은 것만 지운다. 지금 도는 작업은 두고 알린다)

**분담 (2026-09-11 변경, 지휘부 지시)** — 이 PC: 신약 남은 절 → **구약** 구방식 교체 / 새 PC: 구약 남은 절 → **신약** 구방식 교체.
말씀의삶 1기가 구약(욥기·창세기…)부터 읽으므로 빠른 이 PC 가 구약 교체를 진도표 순서로 먼저 한다.
처음 지시는 각자 맡은 쪽 교체였다(이 PC 신약 / 새 PC 구약). 대상은 서버가 판정하므로 어느 PC 가 구방식을 만들었든 상관없다.
새 PC 의 구약 남은 절과 이 PC 의 구약 교체는 겹치지 않는다 — 남은 절은 서버에 파일이 없는 절, 교체는 구방식 파일이 있는 절이다.

**2026-09-12 사고와 복구** — 교체 작업이 로컬 구방식 파일을 '기존 파일'로 합격 처리해 욥 1:1~26:1(해시 139개)의 옛 음원을
'교체본'으로 올렸다(워커를 약 1분 만에 멈춤). 고침: `new_job`(교체 작업은 로컬 파일을 건너뛰지 않음) + `_skip_already_replaced`
(이미 걸린 작업의 '기존 파일' 절을 시작할 때 되돌림). 복구: 서버가 그 139개를 구방식으로 계속 본다(`lib/voiceStudio/legacy.ts`).
- **새 PC 는 스튜디오를 한 번 더 껐다 켤 것** — 신약 교체를 옛 코드로 걸어, 로컬에 신약 구방식 파일이 있으면 같은 일이 난다.
  고친 코드로 돌면 그 작업을 시작할 때 되돌린다(신약 교체는 구약 남은 절 뒤라 몇 주 뒤에 돈다)
- 교체를 모두 마친 뒤 `LEGACY_REUPLOADED` 는 지워도 된다(남아도 무해 — 새 방식으로 다시 올라간 파일은 해당 없음)
...
**마무리 점검 — 전 절 누락 검사** (2026-09-11 추가, 교체까지 끝난 뒤)
새번역 모든 절의 본문 해시를 서버 영희 목록과 대조해 **없는 절만** 모아 한 번 더 만든다. 보류·건너뜀·본문 변경 등
원인과 무관하게 빠진 절을 잡는 마지막 그물이다. 계기: 새 PC 작업 파일에 본문 정정 **이전** 본문으로 보류된
121절이 남아 있다(관리자 화면은 '본문바뀜'으로 감춤) — 풀리지 않으면 그 절은 새 PC 가 만들지 않는다.

**속도 차이** (2026-09-11 실측) — 이 PC 시간당 약 150절, 새 PC 약 50절(서버 업로드 기록, 둘 다 4절 묶음).
생성은 CPU 한 코어에 묶인다: 이 PC 생성 중 GPU 38%, 파이썬 1코어 포화 — 새 PC 는 GPU 가 좋아도 CPU 가 약해 느리다.
이 속도면 새 PC 구약 남은 약 17,900절에 보름 가까이 걸린다 → **이 PC 가 신약을 마치면 구약 뒷부분을 나눠 맡는다**(지휘부 확인 필요).
→ (2026-09-11 갱신) 같은 8절 측정에서 새 PC 는 배치 8 로 이 PC 와 비슷했다(시간당 498 : 476, 생성만) — 스튜디오 화면 부담을
고치고 새 PC 를 배치 8 로 돌린다. 지휘부는 이 PC 에 구약 뒷부분 대신 **구약 교체**를 맡겼다(위 분담). 새 PC 실제 속도는 배치 8 적용 뒤 다시 본다.

**조각 끝 재검사** (2026-09-11 추가, 마무리 점검과 함께) — 나눠 만든 절(음원 안에 0.15초 이음새)의
중간 조각 끝이 짧은 것을 다시 만든다. 이 PC 새 방식 합격 1,466절 중 23절(마 18:18·25:44, 요 13:20, 행 27:9·27:17·27:27 …).
2026-09-11 부터는 조각마다 끝을 검사하므로 새로 생기지는 않는다. 서버 파일은 동결 뒤라 교체 대상이 아니므로 지우고 다시 만든다.
새 PC 파일은 서버 mp3 에서 이음새를 찾아 가려낸다. 절 끝을 최선본으로 받아들인 절(끝 150ms 미만, 약 13%)도 같이 볼지 판단.

**재생성 재요청 한계** — 같은 절에 '재생성 요청'을 두 번 해도 PC 는 한 번만 처리한다(`regen_applied`, 무한 반복 방지).
창 7:14·35:27 이 이 상태(9/9 요청 → 다시 보류). 들어보고 '이대로 사용' 하거나, 요청 시각 기준으로 다시 집도록 개선.

**밤사이 멈춤** — 2026-09-11 새벽 두 PC 가 함께 멈췄다(이 PC 02:57, 새 PC 01:58 마지막 업로드, 이 PC 06:55 재부팅).
Windows 업데이트 자동 재시작 의심. 생성 PC 는 '사용 시간'을 넓혀 두거나 아침에 재개 여부를 확인한다.
