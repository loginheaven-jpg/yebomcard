...
- **네 목록 편집 화면(수퍼어드민) — 아직 없다**(2026-09-19 확인). 설계는 '관리자 화면에서 목사님이 고친다' 였으나
  만들지 않았다. 지금은 개발 쪽이 DB(`qa_lists`)에 넣는다 — 2026-09-19 이단 목록에 말씀보존학회 · 세광중앙교회(김노아) 추가.
  만들면: 이단 · 가정교회 · 위기 창구 · 삶공부 네 칸, 켜기/끄기 · 순서 · 제목/본문/근거(note), 고친 사람(`updated_by`) 기록
- **선별이 고른 구절을 보지 못한다(2026-09-19 발견, 개선 제안 · 결정 대기)** — 선별 모델은 질문 글만 받아서
  '무슨뜻인가?' · '이게 무슨 말이에요' 처럼 구절에 기대는 짧은 질문을 **거절**한다(운영 기록 #15, 요한복음 14:20).
  답변 쪽은 본문을 받으니 선별만 통과하면 제대로 답한다. 시험: 선별에 **'고른 본문: 책 장:절' 한 줄**을 붙이고 §A 에
  규칙 두 줄(짧은 질문은 그 본문에 대한 것 · 위기는 교인의 글로만 판단)을 더하면 구절 질문 40/40(지금 35/40) ·
  기존 선별 42/42 · 가르기 48/48 · 위기 놓침 0. **본문까지 붙이면 욥 3:11 '무슨 뜻인가요?' 를 위기로 오판**해 주소만 붙인다
