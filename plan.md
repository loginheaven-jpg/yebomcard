...
## AI 캐시 비용 관측 (2026-09-27)
- 응답 캐시의 모델 분리는 게이트웨이 소스에서 확인. 운영 캐시 통계 조회는 사용자 승인까지 받았으나 지정한 `C:/dev/ai-gateway/.env` 파일이 없어 미조회. 관리자 통계 접근 경로가 확보되면 적중률을 확인한다.
- 게이트웨이가 현재 input/output 합계만 반환한다. 공급자별 cached-input/cache-write 토큰을 전달·기록한 뒤 실제 비용과 TTL을 평가할 것. 측정 없이 장기 캐시를 늘리지 않는다.
- 이번 앱 변경은 Flash·Luna / Flash·Luna·Terra 구성, 무료 검색어·색상 배경, DB 선점 중복 방지를 완료했다. 상세는 architecture.md 2026-09-27.

## 성경 질문 함께보기 — 적용 대기와 지켜볼 것 (2026-09-21)

**DB 마이그레이션은 사람이 적용해야 한다** — `scripts/migration-qa-share.sql`(Supabase SQL Editor).
적용 전에는 저장은 되지만 공유가 안 되고(칸이 없다), 나의 질문 화면에 공유 단추가 안 뜬다.

- **지켜볼 것 — 익명이 익명인가.** 교회 규모에서 구절+내용의 조합이 사람을 가리킬 수 있다.
  신고가 들어오거나 "누구 질문인지 알겠더라" 는 말이 나오면 **기본 공개를 기본 비공개로 뒤집는 것**을
  다시 논의한다(코드에서는 `setQaSaved` 의 기본값 한 자리와 체크박스 라벨만 바꾸면 된다)
- **보류 — 함께보기 정렬.** 지금은 최신순(고른 절과 겹치는 것 먼저). 질문이 쌓이면
  '많이 본 순' 이나 '목사님이 고른 것 먼저' 가 필요할 수 있다. 한 절에 열 개를 넘기 시작하면 그때 본다
- **보류 — 본문 아래 줄에도 함께보기.** 지금은 질문 창 안에서만 보인다. 읽는 중에 보이면 좋겠다는 말이 나오면,
  절마다 **한 개**만 접힌 줄로 얹는 방식을 검토한다(전부 얹으면 말씀보다 질문이 길어진다)

## 진도표 여러 개 — 적용 대기와 지켜볼 것 (2026-09-20)

코드는 올렸다. **DB 마이그레이션은 사람이 적용해야 한다** — `scripts/migration-reading-plans.sql`
(Supabase SQL Editor). 적용 전에는 목록이 `ready:false` 로 돌아와 표준진도표만 쓰이고 만들기 단추가 꺼진다.
적용 뒤 운영에서 확인할 것: 진도표 만들기 → 그룹에 붙이기 → 다른 계정으로 회차 체크 시 **잠김**,
수퍼어드민 완전 삭제 시 그 그룹이 **해제**로 보이는지.

- **보류 — 진도표 목록이 길어질 때** 검색·정렬(많이 쓰는 순)을 넣을지. 지금은 교회 하나 규모라 목록이 짧다.
  20개를 넘기 시작하면 그때 본다(지금 넣으면 쓰지도 않을 화면이 는다)
- **보류 — 회차 안 절 단위 편집**. 표준진도표에는 `왕하 15:23-17:41` 같은 절 범위가 있지만,
  만들기 화면은 **장 단위**로만 나눈다(글로는 절을 적을 수 있다). 장 단위 읽음 판정과 어긋나는 것을
  교인이 직접 만들게 할 이유를 아직 못 찾았다
- **지켜볼 것 — AI 초안 품질**. 실제 게이트웨이로 여섯 줄을 재 보니 다 통과했지만
  ("요한복음 30일·주5일" → 21회차), 엉뚱한 입력("오늘 날씨")에도 성경 통독 15회차를 만들어 준다.
  사람이 저장을 눌러야 하니 해는 없다 — 거절 문구가 필요한지는 교인 사용을 보고 정한다

---

## ~~음원 절 끝이 짧게 끊기는 절~~ — 해결 (2026-09-12 제기 → 2026-09-13 조치)

실험으로 **조건은 답이 아니라는 것**을 확인했다(끝 공백·줄바꿈·온도·강제 조각·문장부호 끄기 모두 무효.
같은 본문을 두 번 넣어도 230ms 와 40ms 가 나온다 — 끝 길이는 생성마다 다시 뽑는 제비다). 그래서
**더 뽑는 쪽**으로 갔다: 통째로 만드는 절에도 끝 검사를 붙이고(`engine.synth_batch`), 끝 전용 재시도
한도를 3 → 6 으로 올렸다(`jobs.END_RETRY_MAX`). 이미 올라간 1,132절은 세 번째 덮어쓰기 창으로 다시
만든다. 자세히는 architecture.md 2026-09-13 항목.

- **지켜볼 것** — 다시 만든 뒤에도 기준 미달로 남는 절이 얼마나 되는가. 여섯 번을 뽑아도 안 되는 절
  (같은 자리에서 계속 급히 닫는 절)이 모이면 그 본문들의 공통점을 따로 본다
- **새 PC 반영** — 스튜디오를 다시 켜야 고친 엔진(끝 검사·검수 연속 8자·교체 사고 고침)을 집는다

## 성경 질문 기능 — 만들기 전에 정해 둔 것 (2026-09-18)

교리 기준과 화면 시안까지 지휘부 재가를 받았다. 문서는
...
- **원칙 문서의 자리(지휘부 결정)** — 교리 기준 본문은 저장소에서 읽어 `system_prompt` 로 보내고
  (`outputFileTracingIncludes` 에 문서를 포함시켜야 한다), **이단 목록·가정교회 자료·위기 상담 창구·삶공부 과정 이름**
  네 가지는 DB 에 두고 관리자 화면에서 고친다. 답변 기록에 `PROMPT_VERSION` 과 목록 갱신 시각을 함께 남긴다
- **사람이 확인해야 남는 것** — ① 이단 목록의 '무엇이 갈라지는가' 문구를 합신 결의문으로 맞추기(특히 인터콥·다락방)
  ② 가정교회 세 축·네 기둥 문구를 사역원 표현으로 맞추기 ③ 삶공부 과정 이름
  ④ 위기 창구의 문자·채팅 가능 여부(전화를 못 거는 사람이 위기에 더 많다)
  — VIP 는 2026-09-18 정해졌다(전도 대상자 + 등록 전 새신자)
- **지난 설교(드라이브 .txt 50 편)** — 1단계는 **AI 에게 설교를 주지 않는다.** 앱이 구절→설교 색인으로
  '이 구절을 다룬 우리 교회 설교' 카드를 답 옆에 붙인다(`sermons`·`sermon_refs`). 카드에 적용 질문은 넣지 않고,
  2025-07 이전 설교는 글이 없어(유튜브만) 뺀다. **들여오기는 하루 한 번 자동**(Vercel cron)으로 돈다 —
  파일이 월·화·수로 흩어져 올라오기 때문. 인증은 TTS 가 쓰는 `GCP_SERVICE_ACCOUNT_JSON` 을 scope 만 바꿔
  그대로 쓰므로 **새 비밀값은 `CRON_SECRET` 하나뿐** — [docs/BIBLE_QA_SERMONS.md](docs/BIBLE_QA_SERMONS.md)
- **사람이 드라이브에서 해 주실 일** — 설교 .txt 만 담은 폴더를 따로 만들어 서비스 계정에 보기 권한 공유.
  지금 폴더에는 교인명부·재정·당회록·키 파일이 함께 있어 통째로 공유하면 앱 열쇠로 그것까지 읽힌다
- **만든 것(2026-09-18)** — 서버(선별·답변·기록) · 질문창 UI · 저장과 절 표시 · 수퍼어드민 기록 화면 ·
  음성 입력 · 다른 AI 로 이어가기 · 설교 들여오기와 카드까지 모두 배포했다.
  ~~① `scripts/migration-bible-qa.sql` 실행~~ — **2026-09-18 적용 완료**(Supabase, 점검 창구 `ok: true`).
  **남은 사람 손** — ② 드라이브 설교 폴더를 서비스 계정에 보기 권한 공유 ③ Vercel 환경 변수 `CRON_SECRET`
  (없으면 설교 자동 들여오기가 막혀 수퍼어드민이 손으로만 돌린다 — 점검 창구 `cron.secret_set`)
- ~~**AI 게이트웨이 개선 의뢰(2026-09-18)**~~ — **2026-09-19 반영 완료.** 같은 질문 · 세 칸 동시로 다시 재니
  Gemini 2.5 · ChatGPT 4.7 · Claude 10.7초(중앙값, 예전 30.8 · 16.8 · 46.2초), 선별 0.9초. 모델도 바뀌었다
  (gemini-3.8-flash · gpt-5.6-terra · claude-sonnet-5, 게이트웨이 담당 · 지휘부 결정)
- **칸은 앱이 정한다(2026-09-19 적용)** — AI 고르기를 없애고 늘 Gemini · ChatGPT 두 칸, 교리가 걸린 질문이면 Claude 까지.
  **지켜볼 것**: ① 실제 질문 가운데 `doctrine` 비율(`ai_questions.gate_raw`) — Claude 비용이 여기에 비례한다
  ② 한 달 뒤 Claude 비용이 부담이면 **프롬프트 캐시**(게이트웨이 쪽 작업, 품질 손실 없음)부터 —
  Claude 값의 86% 가 교리 프롬프트(약 17,000토큰)를 매번 읽는 입력값이다. 5분 안에 Claude 질문이 약 22% 이상 이어져야 이익
- ~~**네 목록 편집 화면(수퍼어드민)**~~ — **2026-09-19 만듦**(`/admin/qa-lists`). 이단 목록에 말씀보존학회 ·
  세광중앙교회(김노아) 추가. 가정교회 자료 14줄을 문서에서 DB 로 옮김(규칙은 문서에 남김).
  **남은 것**: 삶공부 과정 이름이 비어 있다 — 교회가 쓰는 과정 이름(생명의삶 등)을 화면에서 넣어 주셔야 가정교회 자료가 붙는다
- ~~**선별이 고른 구절을 보지 못한다**~~ — **2026-09-19 고침**(지휘부 승인). 선별에 '고른 본문: 책 장:절' 한 줄 +
  §A 규칙 두 줄. 실제 앱 코드로 구절 질문 40/40 · 기존 선별 42/42(위기 18/18) · 가르기 46/48(요3:16 에 붙인
  '아멘은 무슨 뜻' 을 교리로 — Claude 가 한 칸 더 붙는 안전한 쪽)
- **후보로 둔 모델(2026-09-19 측정, 넣지 않음)** — 같은 교리 프롬프트 · 가린 채점(대결과 무관한 심사자 셋):
  - **Mistral Large 3** — 값은 Gemini 와 같고(1천 건 $5.2) 6.3초, 점수는 Claude 보다 약 1.2점 낮다(교리 질문에서 특히).
    심각한 문제 0건. 넣으려면 게이트웨이에 Mistral 추가 · 답을 감싼 ``` 벗기기 · 약관(학습 사용) 확인
  - Mistral Medium 3.5 · Small 4, Claude Haiku 4.5 — 성경에 없는 문장을 인용하는 등 심각한 문제가 여러 건. 쓰지 않는다
  - **DeepSeek Flash**(생각 끔) — 3.2초, 1천 건 $0.6~1.1(캐시 적중 시). 가린 채점은 하지 않았다.
    **데이터가 중국 서버에 저장된다** — 교인 질문에 쓸지는 지휘부가 정할 일
  - Gemini · ChatGPT 값이 오르거나 다른 회사가 하나 더 필요할 때 다시 본다. 시험 키는 `.env.local`(`MISTRAL_API_KEY` · `DEEPSEEK_API_KEY`)
...
상단 두 줄 → 한 줄, 플레이어 인라인화까지 배포했다(architecture.md 같은 날 항목). 여기 적은 것은 지켜볼 것과 미룬 것이다.

- **본문 높이 상수 미세 조정** — `browseMaxHClass` 의 값은 옛 상수에서 계산한 추정치다. 기기에서 보고 본문 끝줄이
  잘리거나 여백이 남으면 8px 단위로 맞춘다
- **장 이동 화살표를 뺀 뒤의 반응** — 고령 교인이 장을 넘기는 법(본문 스와이프·제목 탭)을 못 찾으면, 한 번만 뜨는
  안내 토스트나 제목 옆 작은 ▸ 를 검토한다. 지금은 넣지 않았다(폭이 없다)
- **스와이프 기준값 기기 확인** (2026-09-17 개편) — `SWIPE_*` 는 흔히 쓰는 값에서 잡았고 실기기로 다듬지 않았다.
  지켜볼 것: ① 세로로 읽다가 **뜻하지 않게 장이 넘어가는가**(그러면 넘김 거리 50px·튕김 속도 0.3 을 올린다)
  ② iOS 사파리에서 가로로 밀 때 본문이 **위아래로 같이 흔들리는가**(`touch-action` 이 안 먹는 것 — 그때는
  감싼 div 에 non-passive `touchmove` 로 막는 길이 있으나 세로 스크롤이 무거워질 수 있어 미뤘다)
  ③ 본문 안에서 두 손가락 확대가 여전히 되는가
- **메뉴 개편 뒤 지켜볼 것** (2026-09-17 성경 탭·말씀의삶 탭·⋮ 정리)
  ① '목차' 이름표가 사라져, 예배 중 "요한복음 3장 펴세요" 때 목차를 못 찾는 교인이 있는가 — 있으면 성경 탭을
  누를 때 짧은 안내('읽는 중에 다시 누르면 목차')를 한 번 띄우는 것을 검토한다
  ② **초대링크 운영 확인**: 로그인한 폰에서 링크 → 곧바로 참여·진도표 / 로그인 안 한 폰에서 링크 → '로그인하고 참여'
  → 로그인 → 말씀의삶이 저절로 열리고 참여 / 카톡 공유 창에 문구와 링크가 함께 가는가 — 로컬은 SSO 가 없어 못 봤다
  ③ **로그인돼 있으면 링크만 눌러도 묻지 않고 참여**한다(지휘부: '클릭하고 로그인만'). 참여하면 그룹 멤버에게 이름과
  진도가 보인다 — 모르는 사람이 보낸 링크로 원치 않게 참여하는 일이 생기면 '참여' 한 번을 묻도록 바꾼다
- **`CardBuilder` 진입로 없음** (2026-09-17) — 설정 '도구' 의 성경카드가 유일한 입구였다. 카드는 절 선택 →
  '이 말씀으로 카드 만들기' 로 만들므로 page.tsx 연결만 떼고 `components/CardBuilder.tsx` 는 남겼다.
  여러 장에서 모은 절로 카드를 만드는 용도가 따로 있었는지 보고, 없으면 파일을 지운다
- **말씀의삶 머리줄** (2026-09-17 머리줄 통일에서 뺌, 지휘부) — 성경 화면 다섯 곳은 `[두 줄 제목] … [40px 조작]`
  한 틀이 됐지만 말씀의삶은 자기 머리줄(작은 'YEBOM' + 기울인 '말씀의삶' + 진도표·그룹 탭)이다. 뒤로 가는 버튼도 없다.
  맞춘다면 `[‹ 말씀의삶 / 91회차 중 N] … [진도표|그룹]`. 종이 질감으로 따로 짠 화면이라 보고 나서 정한다
- **⋮ 로 옮길 다음 기능** — 지휘부가 이 자리를 기능 메뉴 이사처로 지정했다. 후보: 대역 켜고 끄기, 절 번호 표시,
  자동 다음 장, 책갈피 추가. 옮길 때마다 상단 폭이 그만큼 는다
- **만들지 않기로 한 것** — 역본 선택 바텀시트와 장 선택 시트(작업지시서 §5). 지금은 네이티브 select 와 목차 단계를
  그대로 쓴다. OS 선택기가 전체 이름·접근성·다크모드를 공짜로 주고, 새 시트는 딤·하드웨어 백까지 따라온다
- **말씀의삶 모드의 PlanHeader** — 그 모드에서는 위에 한 줄이 더 붙어 여전히 두 줄이다. 이번 범위 밖으로 두었다

## SSO 로그인 개선 — 남은 몫 (2026-09-12, 작업지시서 v2 이후)

세션 단일화·체크인(연장)·로그아웃 개편·공용기기 혼입 차단·관리자 라우트 서버 재검증까지는 작업했다
(2026-09-12 배포 완료 — `06979a4`). 여기 적은 것은 **이번에 하지 않기로 한 몫**이다.

- **읽기·쓰기 라우트의 세션 판독 통합** — 세션 쿠키를 직접 푸는 곳이 라우트마다 복사돼 약 14곳이다.
  관리자 관문만 `lib/auth/verifySession` 으로 확인하게 했고, 나머지는 손대지 않았다. 로그인 쓰기까지
  넓히려면 이 통합이 먼저다(승인 취소된 교인의 쓰기, 진도 전체 삭제, 그룹 가입 등이 그때 막힌다).
  쓰기 라우트는 **`session_version` 만 보는 경량판**이면 충분하다 — 왕복 1회, 60초 캐시로 부담이 작다
- **낡은 쿠키 사본의 수명** — 교적부가 '로그인 유지' 세션을 **400일**로 봉인한다. 브라우저는 체크인 401 에서
  쿠키가 지워지지만, 밖으로 복사해 둔 쿠키는 그대로 통한다(이 저장소는 음원 PC 등록 때 쿠키를 복사하도록
  안내한다 — `voice/README.md`). 관리자 기능은 위 재검증으로 막았고, 나머지 개인 데이터 라우트가 남았다
- ~~**이메일 화이트리스트 관리자 경로**~~ — 2026-09-12 제거. 소스에 박혀 있던 기본 이메일은 **교적부에 없는
  계정**이라 지금은 아무 권한도 주지 않으면서, 그 이메일이 나중에 교적부에 등록되면 교적부가 회수할 수 없는
  관리자가 되는 잠복 경로였다(게다가 `NEXT_PUBLIC_` 이라 클라이언트 번들에 노출). 비상 진입로는 서버 전용
  `ADMIN_EMAILS`(기본값 없음, `lib/auth/verifySession.ts`)로 옮겼다 — 서버 API 만 열리고 관리자 메뉴는 안 뜬다.
  운영(Vercel)에 `NEXT_PUBLIC_ADMIN_EMAILS` 가 설정돼 있다면 이제 효력이 없으니 함께 지울 것
- **관리자 등급이 11명** (2026-09-12 실측 — admin 10 · super_admin 1, 전원 승인) — 이 11명 모두가 성경 본문
  전역 수정, 보류 음원 영구 삭제, 180일 기기 토큰 발급을 할 수 있다. 예봄성경 관리자 기능을 `super_admin`
  으로 좁힐지는 지휘부 판단 사항
- **`is_approved` 전면 검사** — 관리자 관문에서만 본다. 승인 취소된 교인의 일반 쓰기는 여전히 통과한다
- **인증 없는 유료 엔드포인트** — 2026-09-12 최소 호출 제한을 넣었다(`lib/rateLimit.ts` — 주소별·10분 창:
  이미지 생성 10 · 이미지 정리 20 · 주제 추천 30 · 사진 검색어 30 · 사진 검색 90). 인증은 걸지 않았다.
  비로그인 교인의 카드 만들기를 막지 않으려는 것이다. **남은 한계**: 세는 곳이 서버 인스턴스 메모리라
  인스턴스가 여럿이면 실제 한도가 그 배수가 되고 인스턴스가 자면 초기화된다. 공유 카운터를 두려면 테이블이
  필요하다(스키마 변경 승인 사안). `/api/tts` 는 교인이 성경을 읽는 정상 경로라 제한하지 않았다 — 필요하면
  공유 캐시 적중률과 함께 따로 판단
- **`vs1` 기기 토큰 180일·무상태** — 폐기는 R2 `revoked.json` 뿐이고 단일 `SESSION_SECRET` HMAC 이다
- (교적부 몫) 로그인 `redirect` 는 `.pages.dev` 전체를 허용하는데 로그아웃 `return` 은 `radio-axi.pages.dev`
  하나만 허용한다. 카카오 콜백은 또 다르다 — 허용정책 3곳이 제각각
...
> **2026-09-16: 이 절은 낡았다.** 지금 현황·운영·남은 일은 [docs/VOICE_OPERATIONS.md](docs/VOICE_OPERATIONS.md) 를 본다.
> 아래는 이력으로 남긴다(교체가 끝나면 할 일 — `DB_VERSION` 올리기 · 전 절 누락 검사 — 은 그 문서 §11 로 옮겼다).
