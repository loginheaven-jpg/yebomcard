...
| 항목 | 상태 |
|---|---|
| 욥기 (1,070절) | 생성 완료 — 합격 1,069 / 보류 1 |
| 욥기 R2 업로드 | **완료** — `tts/v1/ko/f4/` 1,057개 · 96.2MB (중복 본문 병합으로 1,069절 → 1,057파일) |
| 창세기 (1,533절) | 생성 중 (약 6시간) |
...
### 배포 대기 — 영희(f4)를 화면에 노출하려면 push 필요
`app/api/tts/route.ts`(+3줄), `contexts/TtsContext.tsx`(+13줄) 가 **커밋됐으나 미푸시**.
R2 에 음원이 있어도 이게 배포되기 전에는 성우 목록에 '영희'가 뜨지 않는다.
미푸시 커밋: `bf52852`, `c72f68b`, `4847ef9`, `f4d683b`.
