...
- **말씀의삶** — **진도표는 여럿이다**(2026-09-20). 교회 진도표 91회차는 파일 `lib/plans/yebom91.ts` 에 남고
  이름이 **'표준진도표'**(`yebom91`), 그 밖의 진도표는 DB `reading_plans` 에 있고 교인이 만들고 고친다(1인 20개).
  그룹은 하나를 읽고 만든 사람이 바꾼다. **남의 진도표도 목록에 보이고**(틀만 — 진도는 그룹 안에서만) 복사해 갈 수 있다.
  남이 읽기 시작하면 **잠긴다**(이름·설명만). 지우기는 만든 이·운영자, 쓰는 그룹이 있으면 목록에서만 내려가고
  수퍼어드민 `?force=1` 이면 그 그룹은 **해제**(`plan_id → null`, 임의 이동 금지 — 지휘부).
  만들기는 자동 분배(`lib/plans/builder.ts` `autoUnits` — 절 수 기준, 장은 안 쪼갠다) + 줄 고치기 + AI 초안(`gemini-flash`) 한 화면.
  **회차는 `seq` 로 찾고(`units[seq-1]` 금지) 회차 수는 `plan.units.length`**, 진도표 객체는 id 마다 하나(`rememberPlan` — 엔진 캐시가 WeakMap).
  `reading_unit_checks` 는 **`plan_id` 필터 필수**. 표가 없으면 `ready:false` 로 표준진도표만 — `npm run verify:planbuilder`.
...
- **함께보기**(2026-09-21, §B-14) — 저장한 질문을 **이름 없이** 서로 본다. 저장 줄의 '비공개' 체크박스, **기본 공개**.
  보이는 자리는 **질문 창 안**(본문 아래 줄에는 내 질문만 — 인기 절은 수십 개다). **로그인한 교인만.**
  서버 select 에 `user_id`·`user_name` 을 **넣지 않는다**(화면에서 안 그리는 것으로는 모자라다).
  위기·거절은 서버가 한 번 더 막고, 신고는 **공개된 질문의 답까지** 넓혔으며, 수퍼어드민이 `share_hidden_at` 으로 내린다.
  설정 → **나의 질문**(`/my-questions`)은 **본인 것만** — 전체 열람은 `/admin/ai-questions`(열람 기록이 남는다)
- **말로 묻기**(2026-09-21) — '말로 묻기' 한 번이면 끝난다. 말이 끊기면 **3.5초**에 스스로 마감하고
  (마지막 2초는 `곧 마칩니다 3·2·1`), 들은 글을 입력창에 **보인 채로 3초를 센 뒤 스스로 묻는다**
  (`잠깐, 고칠게요` · 입력창을 건드리면 멈춤 — §B-13). 소리 문턱은 **고정하지 않는다**(시작 0.6초로 바닥 소음을 재고,
  시끄러우면 자동 마감을 끈다). **말을 들은 뒤부터** 침묵을 센다 — 안 그러면 생각하는 동안 끝난다.
  녹음 파일 이름은 실제 mimeType 에서 뽑는다(`question.webm` 으로 박으면 **아이폰 사파리는 mp4 라 실패**)
- **질문을 누르면 고른 절을 한 벌 떠서 넘기고 선택은 푼다**(`qaVerses`, 2026-09-21) —
  창이 `selectedVerses` 를 그대로 보면 선택을 푸는 순간 창이 닫힌다
...
| 회차를 `units[seq-1]` 로 찾기 · 회차 수 `91` 을 박기 | 교인이 만든 진도표는 번호가 건너뛴다 → `unitBySeq` · `plan.units.length` |
| `reading_unit_checks` 를 `plan_id` 없이 조회 | 회차 번호가 진도표마다 1부터라 다른 진도표의 체크가 섞인다(화면은 멀쩡해 보인다) |
| 같은 진도표를 매번 새 객체로 만들기 | 엔진 파생값이 객체 키 WeakMap 캐시다 → `rememberPlan(planId, …)` 으로 id 마다 하나 |
