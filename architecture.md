...
> **최종 수정**: 2026-04-01  
> **버전**: 0.3.0  
> **상태**: Phase 1 인프라 완료 → Claude Code 개발 착수 대기
...
| 카드 | 배경 소스 | 생성 방법 |
|------|-----------|-----------|
| 카드 1 | **CSS 그라데이션** | 말씀 키워드 → 12종 프리셋 자동 매칭 |
| 카드 2 | **Unsplash 사진** | 키워드 → 영문 변환 → API 검색 |
| 카드 3 | **AI CSS 아트** | Claude API → 배경 프롬프트 생성 → CSS 렌더링 |
...
[2] 매칭된 키워드 → 배경 소스별 추천
    ...
    - Unsplash: KEYWORD_TO_UNSPLASH로 영문 변환
    - AI: Claude API에 말씀 전달 → CSS 아트 프롬프트
  ↓
[3] 3종 카드 프리뷰 동시 렌더링
```
