...
export default function ScrapList({
  ...
  const tabClass = (t: ScrapTab) =>
    `flex-1 py-1.5 text-xs font-medium text-center rounded-lg transition-colors ${
      tab === t ? "bg-gray-900 text-white" : "text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:bg-gray-800"
    }`;
  ...
  return (
    <div className="w-full max-w-[1200px] mx-auto">
      ...
      {/* Tab toggle */}
      <div className="flex gap-1 mb-4 bg-gray-50 dark:bg-gray-900 p-0.5 rounded-lg">
        ...
        <>
          {myScraps.length === 0 && (
            <div className="flex flex-col items-center justify-center py-16 text-center">
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400 mb-1">저장된 말씀이 없습니다</p>
          ...
          <div className="space-y-2">
            {myScraps.map((scrap) => (
              <div
                key={scrap.id}
                className="bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-800 p-4 hover:bg-gray-50 dark:bg-gray-900 transition-colors cursor-pointer"
                onClick={() =>
                  onSelectScrap(scrap.version, scrap.book_code, scrap.chapter, scrap.verse_start, scrap.verse_end)
                }
              >
                <div className="flex items-start justify-between">
                  <p className="text-sm font-semibold text-gray-800 dark:text-gray-200">{scrap.reference}</p>
                  <button
                    onClick={(e) => { e.stopPropagation(); handleRemove(scrap.id); }}
                    className="p-1 text-gray-400 hover:text-gray-700 dark:text-gray-300 transition-colors shrink-0 ml-2"
                  >
                ...
                {scrap.image_url ? (
                  ...
                  <p className="text-sm text-gray-500 dark:text-gray-400 mt-1 line-clamp-2">{scrap.preview}</p>
                )}
          ...
          <div className="space-y-2">
            {communityScraps.map((scrap, i) => (
              ...
              >
                <div className="flex items-start justify-between">
                  <p className="text-sm font-semibold text-gray-800 dark:text-gray-200">{scrap.reference}</p>
                  <span className="text-xs text-gray-400 shrink-0 ml-2 bg-gray-100 dark:bg-gray-800 px-2 py-0.5 rounded-full">
                    {scrap.scrap_count}명
                  </span>
                </div>
                {scrap.image_url ? (
