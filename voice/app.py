...
def ui_voice_info(name):
    ...
    slot = (f"성우 슬롯 {key} — 만든 절이 예봄성경으로 올라갑니다" if key else
            ...
            "push_voice.py 로 서버에 올리며 슬롯을 정하세요(docs/VOICE_OPERATIONS.md §10)")
...
with gr.Blocks(title="커스텀 보이스 성경 낭독 스튜디오") as demo:
    ...
    with gr.Tab("3. 음원"):
        ...
        gr.Markdown(chr(10).join([
            ...
            " **이 등록은 이 PC 에만 저장됩니다.**",
            "2. **서버에 올리고 성우 슬롯을 정합니다** (한 번). 이 PC 의 명령 프롬프트에서"
            " `python push_voice.py --voice 이름 --voice-key f5 --gender female --session 쿠키값`."
            " 관리자 로그인이 필요합니다(쿠키값: 브라우저로 예봄성경에 관리자로 로그인 → 개발자도구 → 쿠키 →"
            " `saint_record_session`). **이것을 안 하면 다른 PC 가 목소리를 못 받고, 만들어도 올라가지 않습니다.**",
            "3. **다른 PC 는 켤 때 그 목소리를 그대로 받아 갑니다** — 클립을 PC 마다 다시 자르면 음색이"
            " 달라져 책마다 목소리가 바뀝니다. 아래 보이스 정보에 `성우 슬롯 f5` 가 뜨는지 확인하세요.",
            "4. **PC 마다 일을 나눠 줍니다.** 예봄성경 `설정 → 관리자 → 음원 생성 현황` 에서"
            " PC 카드의 *책 맡기기* 로 주거나, 아래 **전체 생성**을 켜면 진도표 순서로 나눠 갖습니다.",
            "",
            "*새 성우를 쓰려면 코드에서 성우 칸을 먼저 열어야 합니다 — 자세한 순서는 docs/VOICE_OPERATIONS.md §10.*",
        ]))
