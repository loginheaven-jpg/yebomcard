...
# 이 프로세스의 워커가 **실제로 만들어 합격시킨** 절 수 — 만드는 속도는 이것으로만 잰다.
# 작업 파일의 합격 누계로 재면 '이미 서버에 있음'·'기존 파일'·관리자 '이대로 사용' 이 섞인다.
# 책이 바뀔 때 수백 절을 한꺼번에 건너뛰면 그만큼 속도가 튀었고(새 PC 시간당 234,390절),
# 켠 직후 뒤에서 작업을 불러오는 동안에도 튀었다(2026-09-16).
_made = [0]
