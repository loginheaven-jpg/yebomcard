...
const LEGACY_REUPLOADED: Partial<Record<string, { before: string; hashes: readonly string[] }>> = {
  f4: {
...
function reuploadedSet(voiceKey: string): Set<string> {
  let s = sets.get(voiceKey);
  if (!s) {
    s = new Set(LEGACY_REUPLOADED[voiceKey]?.hashes || []);
    sets.set(voiceKey, s);
  }
  return s;
}
...
export function isLegacyFile(voiceKey: string, hash: string, lastModified: Date | undefined): boolean {
  ...
  const r = LEGACY_REUPLOADED[voiceKey];
  return !!r && t < Date.parse(r.before) && reuploadedSet(voiceKey).has(hash);
}
