// 직접 Anthropic Claude Haiku(fix-spacing.mjs)로 easy·rnksv 를 잔여 실패 0 까지 수렴 실행.
// 각 pass: easy(체크포인트 이어서·실패 재시도) → rnksv → 잔여 실패 집계.
// 잔여 0 이면 완료, 3회 연속 미감소면 영구 실패 추정 종료. pass 사이 20s(rate-limit 완화).
...
function run(version) {
  return new Promise((resolve) => {
    const p = spawn(process.execPath, ["scripts/fix-spacing.mjs", version], {
      stdio: "inherit",
      env: { ...process.env, CONC: process.env.CONC || "10" },
    });
...
console.log(`[run] 직접 Haiku 수렴 실행 시작 ${ts()} (CONC=${process.env.CONC || "10"})`);
...
while (true) {
  ...
  prevTotal = total;
  if (noProgress >= 3) { console.log(`[run] ⚠ 수렴 — 3회 연속 실패 ${total} 미감소 → 종료 ${ts()}`); break; }
  await sleep(20000);
}
